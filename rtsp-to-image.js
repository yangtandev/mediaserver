...
function isQsvSupported() {
	try {
		...
		console.log(
			'[WARN] QSV hardware acceleration not found. Falling back to software decoding.',
		);
		return false;
	}
}
...
function RTSPToImage(rtsp, type, clientName, useHwAccel = false, retryCount = 0) {
	...
	console.warn(
		`[PERFORMANCE_WARNING] Stream ${id} is configured for high-frequency JPG overwrite. This will cause high I/O and CPU load and is not recommended for production use.`,
	);
	...
	const command = FFMPEG(input)
		...
		.on('end', function () {
			...
			setTimeout(() => {
				RTSPToImage(rtsp, type, clientName, useHwAccel, nextRetryCount);
			}, delay);
		})
		.on('error', function (err, stdout, stderr) {
			...
			if (
				useHwAccel &&
				(ffmpegError.includes('qsv') ||
					ffmpegError.includes('Hardware') ||
					ffmpegError.includes('No device'))
			) {
				console.log(
					`[INFO] RTSP-to-Image HW acceleration failed for ${streamKey}. Retrying with software.`,
				);
